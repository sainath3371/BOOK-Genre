...
handwritten_train_dataset = MathExpressionsDataset(csv_file=csv_path_handwritten_train, images_folder=handwritten_image_folder, vocab=vocab, transform=transform)
# handwritten_val_dataset = MathExpressionsDataset(csv_file=csv_path_handwritten_val, images_folder=handwritten_image_folder, vocab=vocab, transform=transform)

# Update DataLoaders
handwritten_train_loader = DataLoader(handwritten_train_dataset, batch_size=100, shuffle=True)
# handwritten_val_loader = DataLoader(handwritten_val_dataset, batch_size=100, shuffle=False)
...
fine_tune_optimizer = torch.optim.Adam(list(encoder.parameters()) + list(decoder.parameters()), lr=fine_tune_learning_rate)

# Optionally, transfer the state of the old optimizer to the new one
fine_tune_optimizer.load_state_dict(optimizer.state_dict())


# Fine-tune the model
encoder, decoder = train(handwritten_train_loader, encoder, decoder, criterion, fine_tune_optimizer, num_epochs, vocab, device)


bleu_score, predictions = validate_and_calculate_bleu(test_loader, encoder, decoder, vocab, device)
print(f'BLEU Score on Synte test Set: {bleu_score}')
